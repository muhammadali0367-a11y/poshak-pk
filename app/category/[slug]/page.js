...
const SLUG_TO_CAT = {
  ...
  "pret":"Pret / Ready to Wear","luxury-pret":"Luxury Pret",
...
function deriveBadge(tags, name, original_price, price) {
  ...
  if (t.includes("new") || n.includes("new"))       return "New";
...
export default function CategoryPage() {
  ...
  const catName  = SLUG_TO_CAT[slug] || slug;
  ...
  useEffect(() => {
    fetch("/api/brands").then(r=>r.json()).then(j => { if(j.brands) setAllBrands(j.brands); });
  }, []);

  useEffect(() => {
    setLoading(true);
    const [minP, maxP] = parsePriceRange(priceRange);
    const params = new URLSearchParams({ page, category: catName });
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"linear-gradient(160deg,#fdfcfb 0%,#f5f0eb 40%,#ede8e0 100%)", minHeight:"100vh", color:"#2a2420" }}>
      ...
      <nav className="nav">
        ...
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
          {["Lawn","Kurta","Unstitched","Formal","Bridal"].map(cat => (
            <button key={cat} className={`filter-btn ${catName===cat?"active":""}`}
              onClick={() => router.push(`/category/${slugify(cat)}`)}
              style={{ fontSize:".65rem", padding:"5px 10px" }}>
              {cat}
            </button>
          ))}
        </div>
